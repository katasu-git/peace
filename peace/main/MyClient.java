...
public class MyClient extends JFrame implements MouseListener,MouseMotionListener {
		...
		public void run() {
			try{
				...
				if(myNumberInt % 2 == 0){
					...
					myIcon = blackIcon;
					yourIcon = whiteIcon;
					System.out.println("myColor is " + myIcon); //デバック
				...
				while(true) {
					...
					if (inputLine != null) {//読み込んだときにデータが読み込まれたかどうかをチェックする
						...
						if(cmd.equals("MOVE")){//cmdの文字と"MOVE"が同じか調べる．同じ時にtrueとなる
							...
							if(theColor == 0){
								if(theTurn == 3){ //初ターンでパスの場合は白（1）に切り替える
									if(theColor == myColor){
										turnLabel.setText("相手のターンです");
									} else {
										turnLabel.setText("あなたのターンです");
	...
	public void mouseClicked(MouseEvent e) {//ボタンをクリックしたときの処理
		...
		if(theIcon.equals(boardIcon)){
			...
			if(judgeButton(tempy, tempx)){
				...
				out.flush();//送信データをフラッシュ（ネットワーク上にはき出す）する 70行目
				repaint();//画面のオブジェクトを描画し直す
			} else {
				//置けない
				//System.out.println("そこには配置できません");
				agentLabel.setText("そこには置かれへんで。");
			}
			repaint();//画面のオブジェクトを描画し直す
				...
				for(int j=-1; j<=1; j++){
					if(flipButtons(y, x, j, i) >= 1){ //一つ以上裏返せる場合
						...
						for(int dy=j, dx=i, k=0; k<flipNum; k++, dy+=j, dx+=i){
							...
							//System.out.println("ひっくり返す番号はこれです" + theArrayIndex);
	...
	public void setUp(){
		...
		setSize(600,700);//ウィンドウのサイズを設定する
		...
		for(int j=0;j<8;j++){
			...
			buttonArray[i][j].setBounds(i*50+10,j*50+10,50,50);//ボタンの大きさと位置を設定する．(x座標，y座標,xの幅,yの幅）
